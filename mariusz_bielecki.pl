...
:- op(200, fx, ~).
:- op(500, xfy, v). %p v q v r => v(p, v(q, r))
...
%predykat budujący z pozostałych zmiennych krotki (zmienna, x) (wartosciowania uogolnione)
...
%dodatkowo, predykat stara pozbywać się nieznaczących literałów, które już na pewno są fałszywe
